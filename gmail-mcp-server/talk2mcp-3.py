...
async def generate_with_timeout(client, prompt, timeout=30):
    ...
    try:
        ...
        try:
            logger.info(f"Trying with {GEMINI_MODEL}...")
            model = genai.GenerativeModel(GEMINI_MODEL) 
            response = await asyncio.wait_for(
                loop.run_in_executor(
                    None, 
...
async def main():
    ...
    try:
        ...
        if creds_file_path and not os.path.isabs(creds_file_path):
            creds_file_path = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), creds_file_path.lstrip('./'))
        if token_path and not os.path.isabs(token_path):
            token_path = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), token_path.lstrip('./'))
    ...
    You must respond with function calls in this format (one per line):
    ...
    Important:
    - When solving a problem that needs multiple steps, provide ALL function calls needed:
    1. First perform all calculations (strings_to_chars_to_int, then int_list_to_exponential_sum)
    2. Then send email with the results (send-email)
    - Provide multiple FUNCTION_CALL lines if needed
    - When a function returns multiple values, process all of them
    - Do not repeat function calls with the same parameters

    For the query "Find the ASCII values of characters in INDIA, calculate the sum of exponentials of those values, and send the results via email.", you should provide:
    FUNCTION_CALL: strings_to_chars_to_int|INDIA
    FUNCTION_CALL: int_list_to_exponential_sum|[73, 78, 68, 73, 65]
    FUNCTION_CALL: send-email|recipient_id|MCP Calculation Result|The sum of exponentials of ASCII values for INDIA is: [result]
    """
                                    ...
                                    try:
                                        ...
                                        for param_name, param_info in schema_properties.items():
                                            ...
                                            # Normal parameter processing
                                                ...
                                                # Handle array input - check if the value is a comma-separated string
                                                if isinstance(value, str) and ',' in value:
                                                    ...
                                                    # Handle as before for other array formats
...
Sum of exponentials: {last_calculation_result}

[Computed Date / Time: {current_datetime}]"""
                                                
                                                logger.info(f"Updated email message with calculation result: {last_calculation_result}")
                                                logger.debug(f"Final email message: {arguments['message']}")
